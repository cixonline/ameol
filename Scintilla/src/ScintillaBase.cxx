...
void ScintillaBase::ContextMenu(Point pt) {
	if (displayPopupMenu) {
		...
		popup.CreatePopUp();
#ifdef ADOBE
		// Fix: #1465915 JP: LOC-NOTTRANS: The content menu items on the document window aren’t translated. (CL271718)
		AddToPopUp(GetTranslatedText( "Undo" ), idcmdUndo, writable && pdoc->CanUndo());
		AddToPopUp(GetTranslatedText( "Redo" ), idcmdRedo, writable && pdoc->CanRedo());
		AddToPopUp("");
		AddToPopUp(GetTranslatedText( "Cut" ), idcmdCut, writable && currentPos != anchor);
		AddToPopUp(GetTranslatedText( "Copy" ), idcmdCopy, currentPos != anchor);
		AddToPopUp(GetTranslatedText( "Paste" ), idcmdPaste, writable && WndProc(SCI_CANPASTE, 0, 0));
		AddToPopUp(GetTranslatedText( "Delete" ), idcmdDelete, writable && currentPos != anchor);
		AddToPopUp("");
		AddToPopUp(GetTranslatedText( "Select All" ), idcmdSelectAll);
#else
...
#ifdef ADOBE
// Fix: #1465915 JP: LOC-NOTTRANS: The content menu items on the document window aren’t translated. (CL271718)
const char* ScintillaBase::GetTranslatedText( const char* text )
{
	...
	if( text != NULL )
	{
		SCNotification scn	= {0};
		scn.text			= text;
		scn.translation		= sBuffer;
		scn.nmhdr.code		= SCN_TRANSLATETEXT;

		strcpy( (char*)scn.translation, scn.text );
