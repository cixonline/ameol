...
#else GNU
const TCHAR scintillaClassName[] = TEXT("amxctl_editplus");
#endif GNU
...
void ScintillaWin::AddToPopUp(const char *label, int cmd, bool enabled) {
	...
	if( labelLength > 0 )
	{
		int len = ::MultiByteToWideChar( CP_UTF8, 0, label, labelLength, NULL, 0 );
		if( len > 0 )
		{
			LPWSTR wlabel = new WCHAR[len+1];
			::MultiByteToWideChar( CP_UTF8, 0, label, strlen(label), wlabel, len );
			wlabel[len]=0;
			if (enabled)
				::AppendMenuW(hmenuPopup, MF_STRING, cmd, wlabel);
			else
				::AppendMenuW(hmenuPopup, MF_STRING | MF_DISABLED | MF_GRAYED, cmd, wlabel);
			delete[] wlabel;
			return;
		}
	}
...
#else GNU
		wndclass.lpszClassName = L"amxctl_editplus";
#endif GNU
