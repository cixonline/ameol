	...
	for (int margin = 0; margin < vs.margins; margin++) {
		if (vs.ms[margin].width > 0) {
			...
			while ((visibleLine < cs.LinesDisplayed()) && yposScreen < rcMargin.bottom) {
				...
				if (vs.ms[margin].style == SC_MARGIN_CUSTOM) {
					SCNotification scn	= {0};
					scn.line			= lineDoc;
					scn.nmhdr.code		= SCN_CUSTOMDRAWMARGIN;
					scn.cdm.margin		= margin;
					scn.cdm.rect		= rcMarker;
					scn.cdm.surfaceID	= surface->GetSID();
					NotifyParent(scn);
				}
#endif
				if (marks) {
			...
			while (p < ll->numCharsInLine) {
				...
				if (p > 0) {
					if (wrapState == eWrapChar) {
						...
						continue;
#ifndef GNU			
// !!SM!! 2045 commented out below to stop wrapping ignoring style characters
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_GETSELTEXT: {
			...
			int iChar = 0;
#ifndef GNU
			if (selectedText.len && (static_cast<int>(wParam) >= selectedText.len) ) {
#else GNU
			if (selectedText.len) {
#endif GNU
